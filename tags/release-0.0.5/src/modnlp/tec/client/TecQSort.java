...
public class TecQSort {
  ...
  */
  ...
  public void sort(ConcArray conc) {
    SEPTOKEN = conc.SEPTOKEN;
    int hi = 0; 
    while ( conc.concArray[hi] != null )
      hi++;
    hi--;
    quicksort(conc, 0, hi);
  }
  ...
  public void sort(ConcArray conc, int start, int end) {
    quicksort(conc, start, end - 1);
  }

  private void quicksort(ConcArray conc, int p, int r) {
    if (p < r) {
  ...
  private int partition (ConcArray conc, int p, int r) {
    Object pivot = conc.concArray[p].concordance;
    int lo = p;
    int hi = r;
    while (true) {      
      while (comp.compare(conc.concArray[hi].concordance, pivot) >= 0 &&
             lo < hi) {
        hi--;
      }
      while (comp.compare(conc.concArray[lo].concordance, pivot) < 0 &&
             lo < hi) {
        lo++;
      }
      if (lo < hi) {
        // swap concordance lines
        String T = conc.concArray[lo].concordance;
        conc.concArray[lo].concordance = conc.concArray[hi].concordance;
        conc.concArray[hi].concordance = T;
        // swap filenames
        T = conc.concArray[lo].filename;
        conc.concArray[lo].filename = conc.concArray[hi].filename;
        conc.concArray[hi].filename = T;
        // swap file position flages
        int I = conc.concArray[lo].filepos;
        conc.concArray[lo].filepos = conc.concArray[hi].filepos;
        conc.concArray[hi].filepos = I;      
        // swap position on conc.concordances[] flags
        long L = conc.concArray[lo].bytepos;
        conc.concArray[lo].bytepos = conc.concArray[hi].bytepos;
        conc.concArray[hi].bytepos = L;
      }
      else return hi;
    }
  }
